...
@click.option(
    ...
    prompt=True,
    required=True,
    help="The SE conference links you want. Use , to seperate each link.",
)
@click.option(
    "--keyword/--no-keyword",
    default=True,
    type=click.BOOL,
    help="whether generate the keyword ",
)
@click.option(
    "--crawler/--no-crawler",
    default=True,
    type=click.BOOL,
    help="whether scrape the data",
)
@click.option(
    "--retry",
    default=3,
    type=click.INT,
    help="Max retries for crawler errors",
    show_default=True,
)
@click.option(
    "--metasave",
    default="meta.csv",
    help="Path for save the meta information",
    show_default=True,
)
def main(urls, keyword, crawler, retry, metasave):
    urls = urls.split(",")
    if crawler:
        for url in urls:
            ...
            if result:
                ...
                logger.info(f"Skip saving because crawler failed for {url}")
    if keyword:
